...
void create_cell_types( void )
{
	...
	// ADDED CODE HERE
	// std::vector<double> initial_distances(22,0.0);
	cell_defaults.custom_data.add_variable("Distances", "unitless", 0.0);
	cell_defaults.custom_data.add_variable("is_broken_connection", "bool", false);
	cell_defaults.custom_data.add_variable("tau_avg", "unitless", 0.0);
 ...
 { 
    ...
    // Note that you should use a smaller tolerance value than 0.1 but then you need a smaller time steps for computation (PhysiCell.global time steps inculding mechanical time steps)
     if (pCell->custom_data["Freezing_time"]<= pCell->custom_data["tau_t"] && fabs(pCell->custom_data["Freezing_time"]- pCell->custom_data["tau_t"])<0.1&& pCell->custom_data["Active_cell_volume"]>0)
     {
         ...
         // Write some output data - ADDED CODE HERE  
		 ofs.open ("output/rathepatocyte_freezing_time_B400.txt" , std::ofstream::out | std::ofstream::app);
         ofs << PhysiCell_globals.current_time<<", "<< pCell<<",  "<< pCell->custom_data["Freezing_time"]<<","<<pCell->custom_data["Temperature"]<< ",  "<<pCell->custom_data["tau_t"]<< ","<<pCell->custom_data["tau_avg"]<<", "<< pCell->custom_data["is_broken_connection"]<<", "<<pCell->state.ice_state<< std::endl;
         ofs.close(); 

	 // ADDED CODED HERE
	 ofs.open("output/rathepatocyte_distance_after_freezing_B400.txt", std::ofstream::out | std::ofstream::app);
	 ofs << PhysiCell_globals.current_time<<", "<< pCell<< ", "<< pCell->custom_data["Distances"] << std::endl;
	 ofs.close();

	 
    
 
	ofs.open("output/Gillespie_tau_avg10_rathepatocyte_22ncell_B400.txt", std::ofstream::out | std::ofstream::app);
	...
	for (int i = 0; i < (*all_cells).size(); i++) {
    	...
    	if (!pCell) {
        	std::cerr << "Error: Invalid cell pointer at index " << i << std::endl;
        	continue; // Skip invalid cell
    	}

    	std::vector<int> neighbors = populate_my_neighborhood(pCell);
    	double tau_sum = 0.0;
    	double average_tau = 0.0;
   	double count = neighbors.size();
    	ofs << tau_sum << ", " << average_tau << ", " << count << ", ";

    	if (connections_broken(pCell, neighbors)) {
        	for (int neighbor_index : neighbors) {
            		if (neighbor_index >= 0 && neighbor_index < (*all_cells).size()) {
                		Cell* neighbor = (*all_cells)[neighbor_index];
                		tau_sum += neighbor->custom_data["tau_t"];
            		} else {
                		std::cerr << "Warning: invalid neighbor index " << neighbor_index << std::endl;
            		}
        	}
        	average_tau = (count > 0) ? tau_sum / count : 0.0;
        	pCell->custom_data["tau_avg"] = average_tau;
        	pCell->custom_data["is_broken_connection"] = true;
    	} else {
        	pCell->custom_data["is_broken_connection"] = false;
        	pCell->custom_data["tau_avg"] = tau_sum + (pCell->custom_data["tau_t"]);
    	}

    	ofs << pCell->custom_data["tau_avg"] << ", " << pCell->custom_data["tau_t"] << ", " << average_tau << std::endl;
	}
...
// ADDED CODE HERE
bool connections_broken(Cell* pCell, const std::vector<int> &neighbors){
	double count = 0.0;
	for (int i: neighbors)
	{	
		Cell *neighbor = (*all_cells)[i];
		// 0.01 connection breakage = 21.2000000
		// 0.10 connection breakage = 21.088140
		// 0.25 connection breakage = 20.081400
		// 0.3 connection breakge = 20.023440

		if (distance_between(pCell, neighbor) > 20.081400){
			count++;
		}
	}

	double ratio = count/ neighbors.size();
	double rounded_ratio = std::round(ratio * 10) / 10.0;
	pCell->custom_data["is_broken_connection"] = (rounded_ratio != 0);
	return (rounded_ratio != 0);
...
void Gillespie_Model(void)
{
	...
	// ADDED CODE HERE
		...
		double total_distance = 0.0;
			...
			Cell* neighbor_cell = (*all_cells)[neighbors[j]];
	    		total_distance += distance_between(pCell, neighbor_cell);	
		}

		double average_distance = 0.0;
		if (!neighbors.empty()){
			average_distance = total_distance/ neighbors.size();
		}
		pCell->custom_data["Distances"] = average_distance;
	    }
