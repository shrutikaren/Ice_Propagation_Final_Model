...
for i in $(seq 1 100); do
	rm -rf ./my_output_simulations/simulation_${i}
done
...
OUTPUT_DIR="my_output_simulations" 
mkdir -p "$OUTPUT_DIR"
...
for i in $(seq 1 $SIMULATION_NUMBER); do
	...
	cp -rf ./output/rathepatocyte_distance_after_freezing_B400.txt "$SIM_DIR/".
...
# This part of the for-loop is to deal with the iteration of the snapshot
        ...
        for j in $(seq 0 $FILE_NUMBER); do
                PADDED_NUMBER=$(printf "%02d" $j) # 00, 01, 02 ... 89
