...
bool connections_broken(Cell* pCell, const std::vector<int> &neighbors){
	...
	{	
		...
		if (distance_between(pCell, neighbor) > 21.200000){
			count++;
		}
	}
...
void Gillespie_Model(void)
{
	...
	// ADDED CODE HERE
		...
		if (connections_broken(pCell, neighbors)){
		 	double count = static_cast<double>(neighbors.size());

			for (int neighbor_index: neighbors){
				Cell* neighbor = (*all_cells)[neighbor_index];
				tau_sum += neighbor->custom_data["tau_t"];
			}
			average_tau = (count > 0) ? tau_sum / count : 0.0; // Avoid division by zero
			...
			pCell->custom_data["tau_avg"] = static_cast<double>(pCell->custom_data["tau_t"]);
		}
